...
$context_precision_good = get_config('block_multimodalrag', 'context_precision_good') ?: 0.8;
$context_recall_good = get_config('block_multimodalrag', 'context_recall_good') ?: 0.8;
$answer_relevance_good = get_config('block_multimodalrag', 'answer_relevance_good') ?: 0.8;
...
function calculate_enhanced_ragas_metrics($results) {
    ...
    for ($i = 1; $i < count($results); $i++) {
        ...
        if (!empty($generated_answer) && strlen(trim($generated_answer)) > 0) {
            ...
            $chunks_text_only = array_map(function($chunk) { return $chunk['text'] ?? ''; }, is_array($chunks_data) ? $chunks_data : []);
            ...
            $recall_score = calculate_enhanced_context_recall($chunks_text_only, $ground_truth, $question);
            ...
            $faithfulness_score = calculate_enhanced_faithfulness($generated_answer, $chunks_text_only);
...
function calculate_enhanced_context_precision($chunks_data, $ground_truth, $question) {
    ...
    foreach ($chunks_data as $chunk) {
        ...
        $relevance_score = (0.4 * $semantic_relevance) + (0.05 * $question_relevance) + (0.05 * $keyword_overlap) + (0.5 * $chunk_score); #to focus more on chunk score
...
function calculate_enhanced_context_recall($chunks_data, $ground_truth, $question) {
    ...
    $all_chunks_text = implode(' ', $chunks_data);
    
    // Entity coverage
    if (count($ground_truth_entities) > 0) {
        foreach ($ground_truth_entities as $entity) {
            if (stripos($all_chunks_text, $entity) !== false) {
                $covered_entities++;
            }
        }
        $entity_recall = $covered_entities / count($ground_truth_entities);
    } else {
        $entity_recall = 0;
    }
    
    // Concept coverage
    if (count($ground_truth_concepts) > 0) {
        foreach ($ground_truth_concepts as $concept) {
            if (stripos($all_chunks_text, $concept) !== false) {
                $covered_concepts++;
            }
        }
        $concept_recall = $covered_concepts / count($ground_truth_concepts);
    } else {
        $concept_recall = 0;
    }
    
    // If there are no concepts, recall should be based on entities and vice-versa.
    // If both are present, average them. If neither, 0.
    if (count($ground_truth_concepts) > 0 && count($ground_truth_entities) > 0) {
        return ($entity_recall + $concept_recall) / 2;
    } elseif (count($ground_truth_concepts) > 0) {
        return $concept_recall;
    } elseif (count($ground_truth_entities) > 0) {
...
function calculate_enhanced_faithfulness($generated_answer, $chunks_data) {
    ...
    $context_text = implode(' ', $chunks_data);
...
function calculate_enhanced_answer_relevance($generated_answer, $question) {
    ...
    return (0.2 * $intent_alignment) + (0.2 * $semantic_relevance) + (0.6 * $keyword_relevance);
}
...
function calculate_factual_accuracy($generated_answer, $ground_truth) {
    ...
    foreach ($answer_facts as $fact) {
        foreach ($ground_facts as $ground_fact) {
            if (calculate_semantic_similarity($fact, $ground_fact) > 0.4) {
                $correct_facts++;
                break;
            }
        }
...
function extract_entities($text) {
    ...
    // Extract capitalized words (potential proper nouns), minimum 3 letters
    preg_match_all('/\b[A-Z][a-zA-Z]{2,}\b/', $text, $proper_nouns);
    if (!empty($proper_nouns[0])) {
        // Filter out common English words that can start a sentence
        $common_starters = ['The', 'Is', 'Are', 'Was', 'Were', 'A', 'An', 'And', 'Or', 'But', 'In', 'On', 'At', 'To', 'For', 'Of', 'With', 'By', 'What', 'How', 'When', 'Where', 'Why', 'Who', 'This', 'That', 'These', 'Those'];
        $filtered_nouns = array_filter($proper_nouns[0], function($word) use ($common_starters) {
            return !in_array($word, $common_starters);
        });
        $entities = array_merge($entities, $filtered_nouns);
    }
    
    return array_unique($entities);
}

function extract_key_concepts($text, $limit = 10) {
    $stopwords = [
        ...
        'should', 'now', 'd', 'll', 'm', 'o', 're', 've', 'y', 'get', 'goes', 'php'
    ];
        ...
        <div class="response-section">
            ...
            ?>
                            ...
                            <div class="metrics-section">
                                ...
                                <div class="metrics-grid">
                                    ...
                                    <div class="metric-item">
                                        ...
                                        <span class="metric-value <?php echo $metrics['answer_relevance'] > $answer_relevance_good ? 'excellent' : ($metrics['answer_relevance'] > ($answer_relevance_good - 0.1) ? 'good' : ($metrics['answer_relevance'] > ($answer_relevance_good - 0.3) ? 'medium' : 'poor')); ?>"><?php echo round($metrics['answer_relevance'], 3); ?></span>
                ...
                new Chart(distributionCtx, {
                    type: 'doughnut',
                    data: {
                        labels: ['Excellent (>70%)', 'Good (50-70%)', 'Medium (20-50%)', 'Poor (<20%)'],
                        datasets: [{
                            data: [
                                avgScore > 70 ? 1 : 0,
                ...
                function downloadPDF() {
                    ...
                    doc.text('RAG Evaluation Report', 20, 20);
                    ...
                    doc.text('Quality Metrics', 20, 145);
                    ...
                    if (overallScore > 70) grade = 'Excellent';

                    else if (overallScore > 50) grade = 'Good';

                    else if (overallScore > 30) grade = 'Medium';
                    ...
                    doc.save('rag_evaluation_report.pdf');
