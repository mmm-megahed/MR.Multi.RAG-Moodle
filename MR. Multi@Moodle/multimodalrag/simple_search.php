...
$limit = optional_param('limit', get_config('block_multimodalrag', 'default_search_limit') ?: 5, PARAM_INT);
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select id="content_filter" name="content_filter" class="form-control">
                                    ...
                                    <option value="pdf" <?php echo $content_filter == 'pdf' ? 'selected' : ''; ?>>PDF</option>
                        ...
                        if ($result['http_code'] == 200) {
                            ...
                            if ($response_data && isset($response_data['signal']) && $response_data['signal'] === 'vectordb_search_success') {
                                if (isset($response_data['results']) && is_array($response_data['results']) && count($response_data['results']) > 0) {
                                    ...
                                    // ## FIX 2: Enhanced content categorization logic ##
                                    foreach ($response_data['results'] as $idx => $item) {
                                        ...
                                        } elseif ($content_type === 'text' && preg_match('/^\[(\~?)(\d{1,2}:\d{2}(?::\d{2})?)\]/', $text)) {
                                            // If metadata says it's text, but it looks like a transcript, categorize as transcript.
                                            $categorized_results['transcript'][] = ['index' => $idx, 'data' => $item];
                                        } else {
                                            // Otherwise, trust the metadata or default to text.
                                    ...
                                    foreach ($categorized_results as $content_type => $results) {
                                        ...
                                        if ($content_type === 'image') {
                                            ...
                                            foreach ($results as $result_item) {
                                                ...
                                                // ## ENHANCEMENT 1 (Image): Pre-parse image details to get source file for the header ##
                                                $image_name = '';
                                                $source_pdf = '';
                                                $page_num = '';
                                                $context = '';
                                                ...
                                                if (isset($item['score'])) {
                                                    ...
                                                    $match_text = "Match: " . round($item['score'] * 100, 1) . "%";
                                                    if (!empty($source_pdf)) {
                                                        $match_text .= " in <strong>" . htmlspecialchars($source_pdf) . "</strong>";
                                                    }
                                                    echo "<span class='result-score {$score_class}'>{$match_text}</span>";
                                                }
                                                echo "</div>";
                                                echo "</div>";
                                                
                                                // Display parsed image content
                                                if (isset($item['text'])) {
                                                    ...
                                                    if ($context) {
                                                        ...
                                                        echo "<em class='full-context'>" . htmlspecialchars($context) . "</em>";
                                                        echo "</div>";
                                                    }
                                                    echo "</div>";
                                                    echo "</div>";
                                            ...
                                            if ($content_type === 'transcript') $type_name = 'Video Transcript';
                                            elseif ($content_type === 'pdf') $type_name = 'PDF Document';
                                            elseif ($content_type === 'text') $type_name = 'Text Content';
                                            
                                            echo "<div class='content-type-section {$content_type}-section'>";
                                            echo "<h4>" . format_content_type_icon($content_type) . " {$type_name}s</h4>";
                                            ...
                                            foreach ($results as $result_item) {
                                                ...
                                                // ## ENHANCEMENT 1 (Text): Enhanced header with source file and cleaned up breadcrumb ##
                                                ...
                                                if (isset($item['score'])) {
                                                    ...
                                                    if (isset($metadata['source_file'])) {
                                                        $match_text .= " in <strong>" . htmlspecialchars(basename($metadata['source_file'])) . "</strong>";
                                                    }
                                                    echo "<span class='result-score {$score_class}'>{$match_text}</span>";
                                                }
                                                echo "</div>";
                                                
                                                // Context breadcrumb - source file removed to avoid redundancy
                                                echo "<div class='result-breadcrumb'>";
                                                if (isset($metadata['chunk_index']) && isset($metadata['total_chunks'])) {
                                                    echo " <span class='chunk-info'><i class='fa fa-puzzle-piece'></i> Chunk " . ($metadata['chunk_index'] + 1) . " of " . $metadata['total_chunks'] . "</span>";
                                                }
                                                echo "</div>";
                                                echo "</div>";
...
/* Enhanced image details styling */
.detail-item {
    margin: 15px 0;
    padding: 10px;
    background: #f8f9fa;
...
.detail-item.context em {
    color: #495057;
    font-style: italic;
}

.detail-item.context em.full-context {
    display: block;
    white-space: pre-wrap;
    word-wrap: break-word;
    max-height: none;
