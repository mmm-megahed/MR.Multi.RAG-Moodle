...
class block_multimodalrag extends block_base {
    ...
    public function get_content() {
        ...
        if (has_capability('block/multimodalrag:processfiles', $context)) {
            ...
            // Button to process images from PDFs
            $processimagesurl = new moodle_url('/blocks/multimodalrag/process_images.php', [
                'courseid' => $courseid,
                'sesskey' => sesskey()
            ]);
            $this->content->text .= html_writer::link($processimagesurl,
                '<i class="fa fa-image"></i> Process Images',
                ['class' => 'btn btn-secondary btn-sm mb-2 d-block']);
        }
